...
/* v76: PWA standalone swipe fix for favorites */
(function () {
  const FAV_KEY = "meganeFavoritesV65";

  function loadFavsV76() {
    try {
      return JSON.parse(localStorage.getItem(FAV_KEY) || "[]");
    } catch (e) {
      return [];
  ...
  function saveFavsV76(list) {
    try {
      localStorage.setItem(FAV_KEY, JSON.stringify(list));
    } catch (e) {}
  }

  function escapeHtmlV76(s) {
    return String(s).replace(/[&<>"']/g, ch => ({
      "&": "&amp;",
      "<": "&lt;",
      ">": "&gt;",
  ...
  function removeFavoriteV76(item) {
    if (!item) return;
    const list = loadFavsV76().filter(f => f.key !== item.key);
    saveFavsV76(list);
    try {
      if (typeof updateFavoriteToggle === "function") updateFavoriteToggle();
    } catch (e) {}
  ...
  async function shareFavoriteV76(item) {
    if (!item) return;
    const text = `${item.title || ""}｜${item.meta || ""}`;
    try {
      if (navigator.share) {
  ...
  function jumpToDictFavoriteV76(item) {
    try {
      appMode = "dictionary";
      if (typeof data !== "undefined") {
        const wi = data.words.findIndex(w => w.word === item.word);
  ...
  function enableSwipeFavoriteV76(el, item) {
    if (!el || !item || el.dataset.v76SwipeBound) return;
    el.dataset.v76SwipeBound = "1";
    ...
    function end(e) {
      ...
      if (!moved || Math.abs(currentX) < 10) {
        el.style.transition = "transform .22s ease";
        el.style.transform = "translate3d(0,0,0)";
        jumpToDictFavoriteV76(item);
        currentX = 0;
        return;
      }
      ...
      if (currentX < -threshold()) {
        ...
        el.style.opacity = "0";
        setTimeout(() => {
          removeFavoriteV76(item);
          window.renderDictFavoriteList();
        }, 220);
      } else if (currentX > threshold()) {
        shareFavoriteV76(item);
    ...
    // Pointer Events: PWA standaloneで安定しやすい
    el.addEventListener("pointerdown", e => {
  ...
  window.renderDictFavoriteList = function renderDictFavoriteListV76() {
    const listEl = document.getElementById("favoriteList");
    if (!listEl) return;

    const list = loadFavsV76().filter(f => f.type === "dict");
    ...
    // buttonではなくdiv化。iOS PWAでタッチジェスチャーが安定しやすい。
    listEl.innerHTML = list.map(item => `
      <div class="favorite-item" role="button" tabindex="0" data-key="${escapeHtmlV76(item.key)}">
        <span class="favorite-item-title">${escapeHtmlV76(item.title || "Untitled")}</span>
        <span class="favorite-item-meta">辞書｜${escapeHtmlV76(item.meta || "")}</span>
      </div>
    `).join("");

    listEl.querySelectorAll(".favorite-item").forEach(el => {
      const item = list.find(f => f.key === el.dataset.key);
      enableSwipeFavoriteV76(el, item);
    });
  };

  function bindFavoriteOpenV76() {
    const favBar = document.getElementById("randomWord");
    const dialog = document.getElementById("favoriteDialog");

    if (favBar && !favBar.dataset.v76OpenBound) {
      favBar.dataset.v76OpenBound = "1";
      favBar.addEventListener("click", e => {
        e.preventDefault();
        e.stopImmediatePropagation();
        window.renderDictFavoriteList();
  ...
  function bootV76() {
    bindFavoriteOpenV76();
    window.renderDictFavoriteList();
    setInterval(bindFavoriteOpenV76, 600);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootV76);
  } else {
    bootV76();
  }
})();
...
/* v79: favorite scroll safe - tap disabled, long press opens */
(function () {
  const FAV_KEY = "meganeFavoritesV65";
  ...
  async function share(item) {
    const text = `${item.title || ""}｜${item.meta || ""}`;
    try {
      if (navigator.share) await navigator.share({ title: "MEGANE DICTIONARY", text });
      else if (navigator.clipboard) { await navigator.clipboard.writeText(text); alert("コピーしました"); }
      else prompt("コピーしてください", text);
    } catch(e) {}
  }

  function bindSwipe(el, item) {
    if (!el || !item || el.dataset.v79Bound) return;
    el.dataset.v79Bound = "1";

    let sx = 0, sy = 0, dx = 0, dragging = false, moved = false, pid = null, lp = null, opened = false;
    const threshold = () => window.innerWidth * 0.5;
    ...
    function start(x, y, pointerId) {
      sx = x; sy = y; dx = 0; dragging = true; moved = false; opened = false; pid = pointerId ?? null;
      el.style.transition = "none";
      el.style.opacity = "1";
      el.classList.remove("swiping-left", "swiping-right");
      clearLP();
    ...
    function end(e) {
      ...
      if (dx < -threshold()) {
        ...
        el.style.transform = "translate3d(-110%,0,0)";
        el.style.opacity = "0";
        setTimeout(() => { remove(item); window.renderDictFavoriteList(); }, 220);
  ...
  function bindOpen() {
    const favBar = document.getElementById("randomWord");
    const dialog = document.getElementById("favoriteDialog");
    if (!favBar || favBar.dataset.v79OpenBound) return;
    favBar.dataset.v79OpenBound = "1";
    favBar.addEventListener("click", e => {
      e.preventDefault();
      e.stopImmediatePropagation();
      window.renderDictFavoriteList();
...
/* v140: restore favorite left-swipe delete after iPhone performance fix
   Delegated touch/pointer handler. Lower threshold and no recurring timer.
*/
(function(){
  const FAV_KEY = "meganeFavoritesV65";
  if (window.__meganeFavoriteSwipeV140) return;
  window.__meganeFavoriteSwipeV140 = true;
  ...
  function itemFromKey(key){
    return load().find(f => f.key === key && f.type === "dict");
  }
  ...
  // 最後に定義された一覧描画を上書き。古いv75-v83の個別バインドを使わない。
  ...
  const DELETE_THRESHOLD = () => Math.min(92, Math.max(62, window.innerWidth * 0.22));

  function resetTarget(animate=true){
    if(target){
      target.classList.remove("swiping-left", "swiping-right", "longpress-ready", "longpress-charging");
      target.style.transition = animate ? "transform .22s ease, opacity .22s ease" : "none";
  ...
  function move(x, y, ev){
    ...
    if(!moved){
      ...
      moved = true;
      horizontal = Math.abs(dx) > Math.abs(dy) * 1.05;
    }
    ...
    // 左削除を主目的にする。右は軽く抵抗だけ。
    const visualX = dx > 0 ? dx * 0.18 : dx;
    target.style.transform = `translate3d(${visualX}px,0,0)`;
    target.classList.toggle("swiping-left", visualX < -12);
    target.classList.toggle("swiping-right", false);
  }
  ...
  function end(ev){
    ...
    const shouldDelete = horizontal && dx < -DELETE_THRESHOLD();
    const shouldJump = !horizontal && !moved && elapsed < 450;
    ...
    if(shouldDelete){
      ...
      if(ev && ev.stopPropagation) ev.stopPropagation();
      const old = target;
      old.style.transition = "transform .20s ease, opacity .20s ease";
      old.style.transform = "translate3d(-115%,0,0)";
      old.style.opacity = "0";
      const deleteKey = key;
      resetTarget(false);
      setTimeout(() => {
        removeByKey(deleteKey);
        renderList();
      }, 205);
      return;
    }
  ...
  function bindDelegatedSwipe(){
    const listEl = document.getElementById("favoriteList");
    if(!listEl || listEl.__v140SwipeBound) return;
    listEl.__v140SwipeBound = true;

    listEl.addEventListener("touchstart", ev => {
      const t = ev.changedTouches && ev.changedTouches[0];
      const el = ev.target.closest && ev.target.closest(".favorite-item");
      if(!t || !el || !listEl.contains(el)) return;
      start(el, t.clientX, t.clientY, null);
    }, {capture:true, passive:true});
    ...
    listEl.addEventListener("touchend", ev => { end(ev); }, {capture:true, passive:false});
    listEl.addEventListener("touchcancel", ev => { resetTarget(true); }, {capture:true, passive:true});

    listEl.addEventListener("pointerdown", ev => {
      // iOSのtouchはtouch系で処理。PC/マウスのみpointerで処理。
      if(ev.pointerType === "touch") return;
      const el = ev.target.closest && ev.target.closest(".favorite-item");
      if(!el || !listEl.contains(el)) return;
      pointerId = ev.pointerId;
      start(el, ev.clientX, ev.clientY, ev.pointerId);
    }, true);
    ...
    listEl.addEventListener("click", ev => {
      const el = ev.target.closest && ev.target.closest(".favorite-item");
      if(!el || !listEl.contains(el)) return;
      // 横スワイプ直後のclick暴発だけ止める。通常タップはジャンプ。
      const item = itemFromKey(el.dataset.key || "");
      if(item) jump(item);
    }, true);
  }
  ...
  function bindOpen(){
    const favBar = document.getElementById("randomWord");
    const dialog = document.getElementById("favoriteDialog");
    if(favBar && !favBar.__v140OpenBound){
      favBar.__v140OpenBound = true;
      favBar.addEventListener("click", ev => {
        ev.preventDefault();
  ...
  if(typeof render === "function" && !render.__v140FavoriteSwipeHook){
    const oldRender = render;
    render = function(){
      const result = oldRender.apply(this, arguments);
      setTimeout(() => { bindOpen(); bindDelegatedSwipe(); }, 0);
      return result;
    };
...
/* v139: stable favorite list after add - authoritative renderer/binder
   Fixes iOS case where newly-added favorites render with loose gaps and no swipe binding
   until reload. This block runs last and re-binds the current favorite DOM after any redraw.
*/
(function(){
  if (window.__meganeFavoriteStableV139) return;
  window.__meganeFavoriteStableV139 = true;
  ...
  function dictList(){ return load().filter(f => f && f.type === "dict"); }
  function updateStar(){
    try {
      if (typeof updateFavoriteToggle === "function") updateFavoriteToggle();
    } catch(e) {}
    ...
    if (!btn) return;
    try {
      if (typeof currentWord !== "function" || typeof currentGlass !== "function") return;
      const w = currentWord();
      const g = currentGlass();
      if (!w || !g) return;
      const key = `dict:${g.id}:${w.word}`;
      const active = dictList().some(f => f.key === key);
      btn.hidden = false;
      btn.classList.toggle("active", active);
      btn.textContent = active ? "★" : "☆";
    } catch(e) {}
  ...
  function injectStyle(){
    if (document.getElementById("favoriteStableV139Style")) return;
    const style = document.createElement("style");
    style.id = "favoriteStableV139Style";
    style.textContent = `
      ...
      #favoriteList .favorite-item-title,
      #favoriteList .favorite-item-meta{
        display:block!important;
        pointer-events:none!important;
      }
      #favoriteList .favorite-item.swiping-left{
  ...
  function bindSwipe(el, item){
    if (!el || !item || el.__favV139Bound) return;
    el.__favV139Bound = true;

    let sx=0, sy=0, dx=0, pid=null, dragging=false, moved=false, lp=0, opened=false;
    const threshold = () => Math.min(128, Math.max(78, window.innerWidth * 0.28));
    ...
    function start(x,y,pointerId){
      sx=x; sy=y; dx=0; pid=pointerId ?? null; dragging=true; moved=false; opened=false;
      el.style.transition="none";
      el.style.transform="translate3d(0,0,0)";
      el.style.opacity="1";
    ...
    function move(x,y,e){
      ...
      const dy=y-sy;
      if(!moved){
        if(Math.abs(dy)>10) clearLP();
        if(Math.abs(ndx)<10) return;
        if(Math.abs(dy)>Math.abs(ndx)*1.2) return;
        moved=true;
        clearLP();
      }
      dx=ndx;
    ...
    function reset(){
      el.classList.remove("swiping-left", "swiping-right", "longpress-ready", "longpress-charging");
      el.style.transition="transform .28s cubic-bezier(.2,1.35,.5,1), opacity .2s ease";
      el.style.transform="translate3d(0,0,0)";
      el.style.opacity="1";
    }
    function end(e){
      ...
      dragging=false;
      clearLP();
      if(opened || !moved || Math.abs(dx)<10){ reset(); dx=0; pid=null; return; }
      if(dx < -threshold()){
        el.style.transition="transform .2s ease, opacity .2s ease";
        el.style.transform="translate3d(-112%,0,0)";
        el.style.opacity="0";
        setTimeout(()=>{ remove(item); window.renderDictFavoriteList(); },210);
      } else {
        reset();
      }
      dx=0; pid=null;
    ...
    el.addEventListener("pointerdown", e=>{
      // favorite dialog only. Avoid duplicate browser text selection.
      pid=e.pointerId;
      try{ el.setPointerCapture(e.pointerId); }catch(_){}
      start(e.clientX,e.clientY,e.pointerId);
    }, {capture:true});
    ...
    el.addEventListener("pointercancel", e=>{
      clearLP();
      try{ el.releasePointerCapture(e.pointerId); }catch(_){}
      dragging=false; pid=null; dx=0; reset();
    }, {capture:true});
  }
  ...
  window.renderDictFavoriteList = function renderDictFavoriteListV139(){
    injectStyle();
    const listEl=document.getElementById("favoriteList");
    if(!listEl) return;
    const list=dictList();
    listEl.classList.add("favorite-list");
    ...
    listEl.innerHTML=list.map(item=>`
      <div class="favorite-item" role="button" tabindex="0" data-key="${esc(item.key)}">
        <span class="favorite-item-title">${esc(item.title || "Untitled")}</span>
        <span class="favorite-item-meta">辞書｜${esc(item.glassName || "メガネ")}｜${esc((item.meta || "").replace(/^.*?｜/, ""))}</span>
      </div>
    `).join("");

    // Force iOS layout to settle before binding swipe.
  ...
  function bindOpen(){
    const favBar=document.getElementById("randomWord");
    const dialog=document.getElementById("favoriteDialog");
    if(favBar && !favBar.__favV139Open){
      favBar.__favV139Open=true;
      favBar.addEventListener("click", e=>{
        e.preventDefault();
        if(e.stopImmediatePropagation) e.stopImmediatePropagation();
        window.renderDictFavoriteList();
        if(dialog && typeof dialog.showModal==="function" && !dialog.open) dialog.showModal();
        requestAnimationFrame(()=>{ window.renderDictFavoriteList(); });
      }, true);
    }
  }
  ...
  function observe(){
    const listEl=document.getElementById("favoriteList");
    if(!listEl || listEl.__favV139Observed) return;
    listEl.__favV139Observed=true;
  ...
  function boot(){
    ...
    observe();
    window.renderDictFavoriteList();
    // Short startup refresh only. No permanent interval.
    setTimeout(()=>{ bindOpen(); observe(); bindAll(); },100);
    setTimeout(()=>{ bindOpen(); observe(); bindAll(); },500);
  }
...
/* v139: favorite long-press jump restore
   音楽モードなど別モードから下部★で開いた辞書お気に入りでも、
   長押しでその単語へジャンプできるように最終上書きする。
   クリック/タップ単体ではジャンプしない。左スワイプ削除は既存挙動を維持。
*/
(function () {
  const FAV_KEY = "meganeFavoritesV65";

  function loadFavsV139() {
    try { return JSON.parse(localStorage.getItem(FAV_KEY) || "[]"); }
    catch (e) { return []; }
  }
  ...
  function jumpToDictFavoriteV139(item) {
    if (!item || item.type !== "dict") return;
    try {
      window.appMode = "dictionary";

      if (typeof data !== "undefined" && data && Array.isArray(data.words) && Array.isArray(data.glasses)) {
  ...
  function removeDictFavoriteV139(item) {
    if (!item) return;
    try {
      const next = loadFavsV139().filter(f => f.key !== item.key);
      localStorage.setItem(FAV_KEY, JSON.stringify(next));
    } catch (e) {}
  }

  function bindFavoriteItemV139(el, item) {
    if (!el || !item || el.dataset.v139Bound) return;
    el.dataset.v139Bound = "1";
    ...
    const threshold = () => Math.min(170, Math.max(115, window.innerWidth * 0.34));
    ...
    function start(x, y, pointerId) {
      ...
      sy = y;
      dx = 0;
      pid = pointerId == null ? null : pointerId;
      dragging = true;
      moved = false;
      opened = false;
      ...
      timer = setTimeout(() => {
        if (!dragging || moved || opened) return;
        opened = true;
        ...
        el.classList.remove("longpress-charging");
        el.classList.add("longpress-ready");
        setTimeout(() => jumpToDictFavoriteV139(item), 80);
      }, 560);
    }
    ...
    function move(x, y, ev) {
      ...
      if (!moved) {
        if (Math.abs(dy) > 8) clearLongPress();
        if (Math.abs(ndx) < 12) return;
        if (Math.abs(dy) > Math.abs(ndx) * 1.15) return;
        moved = true;
        clearLongPress();
    ...
    function reset() {
      ...
      el.style.transition = "transform .30s cubic-bezier(.2,1.45,.45,1), opacity .22s ease";
      el.style.transform = "translate3d(0,0,0)";
      el.style.opacity = "1";
    }
    ...
    function end(ev) {
      ...
      if (!moved || Math.abs(dx) < 10) {
        // v139: タップだけではジャンプしない。スクロール中の誤爆防止。
        reset();
        dx = 0; pid = null;
        return;
      }
    ...
    // 過去版のclickジャンプを抑止。長押しだけでジャンプさせる。
    el.addEventListener("click", e => {
      e.preventDefault();
      e.stopImmediatePropagation();
    }, true);
  ...
  window.renderDictFavoriteList = function renderDictFavoriteListV139() {
    ...
    const list = loadFavsV139().filter(f => f.type === "dict");
    ...
    listEl.innerHTML = list.map(item => `
      <div class="favorite-item" role="button" tabindex="0" data-key="${escV139(item.key)}">
        <span class="favorite-item-title">${escV139(item.title || "Untitled")}</span>
        <span class="favorite-item-meta">辞書｜${escV139(item.meta || "")}</span>
      </div>
    `).join("");

    listEl.querySelectorAll(".favorite-item").forEach(el => {
      const item = list.find(f => f.key === el.dataset.key);
      bindFavoriteItemV139(el, item);
    });
  };

  function bindOpenV139() {
    const favBar = document.getElementById("randomWord");
    const dialog = document.getElementById("favoriteDialog");
    if (!favBar || !dialog || favBar.dataset.v139OpenBound) return;
    favBar.dataset.v139OpenBound = "1";
  ...
  function bootV139() {
    bindOpenV139();
    if (typeof window.renderDictFavoriteList === "function") window.renderDictFavoriteList();
    setInterval(bindOpenV139, 900);
  }

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bootV139);
  else bootV139();
})();


/* v140: favorite long-press from MUSIC player + theme restore
   下部★を音楽プレイヤー画面から開いた場合でも、MutationObserverで
   お気に入り行を必ず再バインドする。ジャンプ時はmusic系body classを外して
   辞書背景へ戻す。
*/
(function () {
  const FAV_KEY = "meganeFavoritesV65";

  function loadFavsV140() {
    try { return JSON.parse(localStorage.getItem(FAV_KEY) || "[]"); }
    catch (e) { return []; }
  }
  ...
  function restoreDictionaryBodyV140() {
    try {
  ...
  function jumpToDictFavoriteV140(item) {
    if (!item || item.type !== "dict") return;
    ...
    try {
      // appModeがlet/varグローバルでも効くように直接代入も使う。
      appMode = "dictionary";
    } catch (e) {
      try { window.appMode = "dictionary"; } catch (_) {}
    }
    ...
    restoreDictionaryBodyV140();

    try {
      if (typeof render === "function") render("flash");
    } catch (e) {}

    // render後に音楽側のclassが残ることがあるので二段で戻す。
    setTimeout(restoreDictionaryBodyV140, 0);
    setTimeout(restoreDictionaryBodyV140, 80);
  ...
  function removeDictFavoriteV140(item) {
    if (!item) return;
    try {
      const next = loadFavsV140().filter(f => f.key !== item.key);
      localStorage.setItem(FAV_KEY, JSON.stringify(next));
    } catch (e) {}
  }

  function bindFavoriteItemV140(el, item) {
    if (!el || !item || el.dataset.v140Bound) return;
    el.dataset.v140Bound = "1";
    ...
    function start(x, y, pointerId) {
      ...
      timer = setTimeout(() => {
        ...
        el.classList.remove("longpress-charging");
        el.classList.add("longpress-ready");
        setTimeout(() => jumpToDictFavoriteV140(item), 80);
      }, 560);
    }
    ...
    function end(ev) {
      ...
      if (dx < -threshold()) {
        ...
        el.style.opacity = "0";
        setTimeout(() => {
          removeDictFavoriteV140(item);
          renderDictFavoriteListV140();
        }, 220);
      } else {
        reset();
      }
  ...
  function renderDictFavoriteListV140() {
    const listEl = document.getElementById("favoriteList");
    if (!listEl) return;

    const list = loadFavsV140().filter(f => f.type === "dict");
    ...
    listEl.innerHTML = list.map(item => `
      <div class="favorite-item" role="button" tabindex="0" data-key="${escV140(item.key)}">
        <span class="favorite-item-title">${escV140(item.title || "Untitled")}</span>
        <span class="favorite-item-meta">辞書｜${escV140(item.meta || "")}</span>
      </div>
    `).join("");

    listEl.querySelectorAll(".favorite-item").forEach(el => {
      const item = list.find(f => f.key === el.dataset.key);
      bindFavoriteItemV140(el, item);
    });
  }

  window.renderDictFavoriteList = renderDictFavoriteListV140;

  function bindOpenV140() {
    const favBar = document.getElementById("randomWord");
    const dialog = document.getElementById("favoriteDialog");
    if (favBar && dialog && !favBar.dataset.v140OpenBound) {
      favBar.dataset.v140OpenBound = "1";
  ...
  function bindExistingRowsV140() {
    const listEl = document.getElementById("favoriteList");
    if (!listEl) return;

    // 既に別バージョンが描いた行にも、最終長押しを上書きで付ける。
    listEl.querySelectorAll(".favorite-item").forEach(el => {
      const item = loadFavsV140().find(f => f.key === el.dataset.key && f.type === "dict");
      bindFavoriteItemV140(el, item);
    });
  }

  function observeFavoriteListV140() {
    const listEl = document.getElementById("favoriteList");
    if (!listEl || listEl.dataset.v140Observed) return;
    listEl.dataset.v140Observed = "1";

    const mo = new MutationObserver(() => {
      setTimeout(bindExistingRowsV140, 0);
    });
    mo.observe(listEl, { childList: true, subtree: true });
  }
  ...
  function bootV140() {
    bindOpenV140();
    observeFavoriteListV140();
    renderDictFavoriteListV140();
    bindExistingRowsV140();
    setInterval(() => {
      bindOpenV140();
      observeFavoriteListV140();
      bindExistingRowsV140();
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bootV140);
  else bootV140();
})();
