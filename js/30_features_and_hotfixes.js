...
/* vMusicFavLongPressFix: 音楽モードから開いた辞書お気に入りでも長押しジャンプを復旧 */
(function(){
  const FAV_KEY = "meganeFavoritesV65";
  ...
  function findItemByKey(key){
    return load().find(f => f && f.key === key && f.type === "dict") || null;
  }
  ...
  function jump(item){
    ...
    try{ if(typeof updateFavoriteButton === "function") setTimeout(updateFavoriteButton, 80); }catch(e){}
  }

  function bindItem(el){
    if(!el || el.dataset.musicFavLongPressFix) return;
    const key = el.dataset.key;
    const item = findItemByKey(key);
    if(!item) return;

    el.dataset.musicFavLongPressFix = "1";
    ...
    function start(x,y,e){
      ...
      timer = setTimeout(function(){
        ...
        setTimeout(function(){ jump(item); }, 60);
      }, 560);
    }
    ...
    el.addEventListener("pointerdown", function(e){ start(e.clientX, e.clientY, e); }, true);
  ...
  function bindAll(){
    const d = document.getElementById("favoriteDialog");
    if(!d) return;
    d.querySelectorAll(".favorite-item[data-key]").forEach(bindItem);
  }

  const mo = new MutationObserver(bindAll);
  function boot(){
    const d = document.getElementById("favoriteDialog");
    if(d) mo.observe(d, {childList:true, subtree:true});
    bindAll();
