...
(function(){
  ...
  function openDeveloper(){
    ...
    m.innerHTML='<div class="dev-card"><div class="dev-head"><strong>🛠 Developer</strong><button class="dev-close" data-close>×</button></div><div class="dev-tabs"><button data-tab="storage">Storage</button><button data-tab="event">Event</button><button data-tab="observation">Observe</button><button data-tab="reset">Reset</button></div>'
      ...
      + '<div class="dev-section" data-section="reset"><div class="dev-actions"><button data-dev="resetObservation">観測ログ初期化</button><button data-dev="resetLv7Notice">Lv.7通知 初期化</button><button data-dev="testLv7Notice">Lv.7通知 テスト</button><button data-dev="resetHomeGuide">🏠 Home案内 初期化</button><button data-dev="testHomeGuide">🏠 Home案内 テスト</button><button data-dev="resetHomeWelcome">🌸 Home歓迎 初期化</button><button data-dev="testHomeWelcome">🌸 Home歓迎 テスト</button><button data-dev="resetHomeBoth">🏠🌸 両方初期化</button><button data-dev="resetDev">Developerフラグ初期化</button><button data-dev="resetUnlocks">解除テスト初期化</button><button data-dev="resetAllDanger">localStorage全削除</button></div><p class="dev-muted">全削除はカード・お気に入り・解除状態なども消えるので注意。</p></div></div>';
    m.hidden=false;
    ...
    m.querySelectorAll("[data-dev]").forEach(function(btn){ btn.onclick=function(){
      ...
      if(a==="resetHomeGuide"){ if(window.resetMeganeHomeGuideV87) window.resetMeganeHomeGuideV87(); toast("Home案内 初期化"); }
      if(a==="testHomeGuide"){ m.hidden=true; setTimeout(function(){ if(window.showMeganeHomeGuideV87) window.showMeganeHomeGuideV87(); },180); }
      if(a==="resetHomeWelcome"){ if(window.resetMeganeHomeWelcomeV87) window.resetMeganeHomeWelcomeV87(); toast("Home歓迎 初期化"); }
      if(a==="testHomeWelcome"){ m.hidden=true; setTimeout(function(){ if(window.showMeganeHomeWelcomeV87) window.showMeganeHomeWelcomeV87(); },180); }
      if(a==="resetHomeBoth"){ if(window.resetMeganeHomeGuideV87) window.resetMeganeHomeGuideV87(); if(window.resetMeganeHomeWelcomeV87) window.resetMeganeHomeWelcomeV87(); toast("Home通知 両方初期化"); }
      if(a==="resetDev"){ if(window.resetMeganeHomeGuideV87) window.resetMeganeHomeGuideV87(); if(window.resetMeganeHomeWelcomeV87) window.resetMeganeHomeWelcomeV87(); localStorage.removeItem("megane_developer_mode_v1"); localStorage.removeItem("megane_dev_random_test"); var rb=document.getElementById("cardRandomTestButton"); if(rb) rb.style.display="none"; var rp=document.getElementById("cardRandomTestPanel"); if(rp) rp.style.display="none"; toast("Developerフラグ初期化"); }
