...
(function(){
  ...
  function ensureDialog(){
    if(!q("userDefinitionDialog")){
      ...
      d.innerHTML='<div class="userdef-panel"><button id="userDefinitionClose" class="userdef-close" type="button">×</button><div class="userdef-kicker">あなたなら？</div><div id="userDefinitionWord" class="userdef-word"></div><div id="userDefinitionGlass" class="userdef-glass"></div><textarea id="userDefinitionText" class="userdef-textarea" rows="4" maxlength="140" placeholder="あなたには、この言葉がどう見える？"></textarea><div id="userDefinitionCount" class="userdef-count">0 / 140</div><div class="userdef-actions"><button id="userDefinitionSave" type="button" class="userdef-save">保存</button><button id="userDefinitionShare" type="button" class="userdef-cancel userdef-share">シェア</button></div><div class="userdef-note">1単語につき1つ保存 / 保存またはシェアできます</div></div>';
  ...
  function updateCount(){
    ...
    if(c&&ta)c.textContent=String(ta.value.length)+" / 140";
  }
  ...
  function openDialog(){
    ...
    q("userDefinitionText").value=old&&old.text?old.text:"";
    updateCount();
