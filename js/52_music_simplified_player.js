...
(function(){
  ...
  function currentAlbum(){
    if(state.queueMode === "favorites"){
      return { id:"favorites", title:"保護しました♪", desc:"保護済み", cover:"", tracks:favTracks() };
    }
  ...
  function visualAlbumFor(a, t){
    ...
    // 重要: お気に入りは「閲覧だけ」の時 queueMode が album のまま残ることがあるため、
    // a.id / browsingAlbum でも favorites 判定する。
    var favVisual = state.queueMode === "favorites" || state.browsingAlbum === -1 || (a && a.id === "favorites");
    if(favVisual){
  ...
  function renderAlbums(){
    ...
    musicAlbums.forEach(function(a, i){
      ...
      html += '<button type="button" class="music-v7-album-art" data-album="'+i+'">'
        ...
        + '<div class="music-v7-album-copy"><strong>'+esc(a.title)+'</strong><span>'+esc(a.desc || ((a.tracks||[]).length+"曲"))+'</span></div>'
        + '</button>';
    });
    html += '</div>';
    ...
    if(singles.length){
      ...
      singles.forEach(function(x){
        var a = x.album;
        html += '<button type="button" class="music-v7-single-card" data-album="'+x.index+'">'
          + '<div class="music-v7-single-thumb">'+albumCoverHTML(a)+'</div>'
          + '<div class="music-v7-single-copy"><strong>'+esc(a.title)+'</strong><span>'+esc(a.desc || "Single")+'</span></div>'
          + '<em>'+((state.queueMode === "album" && state.album === x.index) ? "▶ 再生中" : "")+'</em>'
          + '</button>';
      });
      html += '</div>';
    }
    ...
    list.querySelectorAll("[data-album]").forEach(function(btn){
      btn.onclick = function(){ switchToAlbum(Number(btn.dataset.album || 0)); };
    });
  ...
  function renderPlayer(){
    ...
    list.innerHTML =
      ...
      + '<div class="music-v7-player-jacket">'+albumCoverHTML(va)+'</div>'
      ...
      + '<button id="musicV7Shuffle" class="music-v7-side-control '+(state.shuffle?"on":"")+'" aria-label="シャッフル">⤨</button>'
      + '<button id="musicV7Prev" class="music-v7-seek music-v7-seek-minimal" aria-label="15秒戻る">-15</button>'
      + '<button id="musicV7Play" class="play" aria-label="再生/停止">▶</button>'
      + '<button id="musicV7Next" class="music-v7-seek music-v7-seek-minimal" aria-label="15秒進む">+15</button>'
      + '<button id="musicV7Repeat" class="music-v7-side-control '+state.repeat+'" aria-label="リピート">'+repeatText()+'</button>'
      + '</div>'
      + '</div>'
      + renderSheet()
      + renderLyrics();
  ...
  function renderSheet(){
    ...
    var rows = trackSource.map(function(t, i){
      ...
      return '<button type="button" class="music-v7-track '+(active?"active":"")+' '+(favMode?"fav-mode":"")+'" data-track="'+i+'" data-track-id="'+esc(t.id)+'">'
        + '<span>'+esc(info.no)+'</span><strong>'+esc(info.title)+'</strong><em>'+(active?"♪":"")+'</em><b data-fav="'+esc(t.id)+'" class="'+(isFav(t.id)?"on":"")+'">★</b>'
        + '</button>';
    }).join("");
    return '<div id="musicV7Sheet" class="music-v7-sheet">'
      + '<div class="music-v7-handle"></div>'
      + '<div class="music-v7-sheet-head"><strong>'+esc(a.title)+'</strong><button id="musicV7SheetClose">×</button></div>'
  ...
  function bindPlayer(){
    ...
    var root = $("musicList");
    if(root && !root.dataset.v7Swipe){
      root.dataset.v7Swipe = "1";
      ...
      root.addEventListener("touchend", function(e){
        if(e.target && e.target.closest && e.target.closest("#musicV7Seek, .music-v7-progress, .music-v7-controls, button")){
          return;
        }
  ...
  function boot(){
    ...
    if(au && !au.dataset.v7Bound){
      ...
      au.addEventListener("timeupdate", function(){ updateProgress(); if(Math.floor(au.currentTime)%7===0) savePos(); });
      au.addEventListener("loadedmetadata", updateProgress);
      au.addEventListener("play", function(){ state.playing = true; updateMediaSessionV8(); updateProgress(); });
      au.addEventListener("pause", function(){ state.playing = false; savePos(); updateMediaSessionV8(); updateProgress(); });
